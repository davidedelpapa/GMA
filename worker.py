        ...
        while (counter < storedCommandsNumber):
            ...
            try:
                ...
                if '>SessionRecords' in query:
                    ...
                    split_q = query.split()
                    cursor.execute("SELECT id FROM SessionRecords WHERE session = '%s' and pkey = '%s'" % (split_q[1], split_q[2]))
                    result_id = cursor.fetchone()
                    if not result_id:
                        # new session, we need to add it
                        cursor.execute("INSERT INTO SessionRecords (ip, firstaccess, lastaccess, session, pkey) VALUES ('%s', '%s', '%s', '%s', '%s')" % (split_q[3], split_q[4], split_q[4], split_q[1], split_q[2]))
                        session_id = cursor.lastrowid
                    else:
                        # session already exists, just update it
                        session_id = result_id[0]
                        cursor.execute("UPDATE SessionRecords  SET lastaccess = '%s' WHERE id = '%s'" % (split_q[4], session_id))
                    #now we must record the tile info as well...
                    cursor.execute(" INSERT INTO TileRecords (sessionid, timestamp, z, x, y) VALUES ('%s', '%s', '%s', '%s', '%s')" % (session_id, split_q[4], split_q[5], split_q[6], split_q[7]))
                    tile_id = cursor.lastrowid
                else:
                    # Direct query, just execute it
                    cursor.execute(query)
            except (mdb.OperationalError, mdb.ProgrammingError) as e:
                logp("[MySQL:Query] %s [ERROR-YELD] %s" % (str(query), str(e.args)))
            #increment counter
            counter = counter + 1
...
try:
    connection = mdb.connect(DBMS_URI, DBMS_USER, DBMS_PASSWD, DBMS_DB, DBMS_PORT)
